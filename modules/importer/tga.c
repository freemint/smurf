...
/*	  FÅr Blockroutinen vorbereitet.						*/
/*	  Andere Palettenoffsets als 0 funktionieren			*/
/*	  wahrscheinlich nicht, ich brÑuchte Testbilder.		*/
...
/*	  öberwachung korrupter Runs die Speicher Åberschreiben	*/
/*	  wÅrden eingebaut.										*/ 
/* Version 0.9  --  12.12.97								*/
/*	  monochrome TGA anderer Programme werden nun nicht		*/
/*	  mehr invertiert angezeigt.							*/
/* Version 1.0  --  12.07.98								*/
/*	  Einige Sicherheitsabfragen auf ungÅltige Headerwerte	*/
/*	  eingebaut.											*/
/* Version 1.1  --  12.09.98								*/
/*	  UnterstÅtzung fÅr umstrittenes aber von manchen Pro-	*/
...
#ifdef GERMAN
#define ERROR1 "UngÅltige Farbtiefe. Bild kann nicht geladen werden."
#define ERROR2 "[2][UngÅltige Werte im Bildheader.|Bild auf eigene Gefahr laden?][Abbruch|OK]"
#define ERROR3 "Nicht genug Speicher um die Orientierung zu Ñndern"
#else
#ifdef ENGLISH
#define ERROR1 "UngÅltige Farbtiefe. Bild kann nicht geladen werden."
#define ERROR2 "[2][UngÅltige Werte im Bildheader.|Bild auf eigene Gefahr laden?][Abbruch|OK]"
...
void read_24Bit(char *buffer, char *ziel, unsigned int width, unsigned int height, char BitsPerPixel)
{
	...
	do
	{
		...
		do
		{
			...
			*ziel++ = *buffer++;
			*ziel++ = *(buffer++ - 2);
			if(skip)							/* Alphachannel Åberspringen */
				buffer++;
		} while(++x < width);
	} while(++y < height);
