...
void X_Scale(char *opic, char *npic, long obpl, long nbpl, int owidth, int height, int nwidth, GARGAMEL *smurf_struct)
{
        ...
        {     
            for(y=0; y<height; y++)
            {
                ...
                for(x=0; x<nwidth; x++)
                {
                    ...
                    *(n_offset++) = (char)((red   * coldiv)>>16);
                    *(n_offset++) = (char)((green * coldiv)>>16);
                    *(n_offset++) = (char)((blue  * coldiv)>>16);
    ...
    {               /* -Interpolieren */
        ...
        for(y=0; y<height; y++)
        {
            ...
            {
                lcount = 0xffff-count;
                *(n_offset++) = (char)(( sred  *lcount + ered  *count )>>16);
                *(n_offset++) = (char)(( sgreen*lcount + egreen*count )>>16);
                *(n_offset)   = (char)(( sblue *lcount + eblue *count )>>16);
                n_offset -= 5;
