...
int do_it(GARGAMEL *smurf_struct)
{
    ...
    signed long x_vekt, y_vekt, o_height, x_dif, y_dif;
    long abstand;
    ...
    col_size = (long)((100L - diffuse_hard) * 6.5 + 50);
    glanz_size = (long)((100L - glanz_hard) / 2.04 + 1);
    
    red   = (char)((long )red   * diffuse_strengh / 100);
    green = (char)((long )green * diffuse_strengh / 100);
    blue  = (char)((long )blue  * diffuse_strengh / 100);
    
    gred   = (char)((long )gred   * glanz_strengh / 100);
    ggreen = (char)((long )ggreen * glanz_strengh / 100);
    gblue  = (char)((long )gblue  * glanz_strengh / 100);
    
    
    if(Obj_Selected(BM_SMOOTH)
            bump_fakt = (long )(bs_diago / 435F * (bm_strengh / 100F) * 1024F);
    else
            bump_fakt = (long )(bs_diago / 360F * (bm_strengh / 100F) * 1024F);
    ...
    for(i=0;i<256;i++)
    {
        *(wurztab + (long)i) = (char)( (float)i * 0.707);
    }
    ...
    {
        n = t*t;
        *(c_offset++) = (char)((long)red   * (n) / coltab_size2);
        *(c_offset++) = (char)((long)green * (n) / coltab_size2);
        *(c_offset++) = (char)((long)blue  * (n) / coltab_size2);
    }
    ...
    for(t=0; t<glanztab_size; t++)
    {
        ...
        nred   =   (int) ((long)(*(c_offset + 0L))  
                     +         (long ) ( (long)gred   * (glanztab_size-t) / glanztab_size));
        ngreen =   (int) ((long)(*(c_offset + 1L))  
                     +         (long ) ( (long)ggreen * (glanztab_size-t) / glanztab_size));
        nblue  =   (int) ((long)(*(c_offset + 2L))  
                     +         (long ) ( (long)gblue  * (glanztab_size-t) / glanztab_size));
    ...
    for (y=0; y<height; y++)
    {
        ...
        for(x=0; x<width;x++)
        {
            ...
            if(x < (width-1))           
              x_vekt += (signed long)(o_height - (signed long)*(g_offset + 1));
            if(y < (height-1))
                y_vekt += (signed long)(o_height - (signed long)*(g_offset + gbpl));
            if(x > 0)   
                x_vekt -= (signed long)(o_height - (signed long)*(g_offset - 1)); 
            if(y > 0)   
                y_vekt -= (signed long)(o_height - (signed long)*(g_offset - gbpl));
            ...
            x_vekt = (x_vekt * bump_fakt) >>10;
            y_vekt = (y_vekt * bump_fakt) >>10;
