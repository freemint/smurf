...
int floyd_steinberg68030(SMURF_PIC *picture, DITHER_DATA *dither, 
            char *output);

void read_24_68030(void);
void read_16_68030(void);
...
int floyd_steinberg68030(SMURF_PIC *picture, DITHER_DATA *dither, char *output)
{
    ...
    int ClipTable[2048];
    register int xpalette[780], *destpal;
    char *actual_pic;
    int *clipaccess;
    int nidx;
    int x, y, t, stpix_counter=0;
    int red_diff, grn_diff, blu_diff;
    unsigned int dest_planes;
    unsigned int width, height, owidth;
    long planelen;
    ...
    */
    if(picture->format_type == FORM_STANDARD)   
        read_pixel = read_8_68030;
    else
    {
        if(picture->depth == 24)        read_pixel = read_24_68030;
        else if(picture->depth == 16)   read_pixel = read_16_68030;
        else if(picture->depth == 8)    read_pixel = read_8_68030;
    }




    line = malloc(width+10);
    service->reset_busybox(0, "Floyd Steinberg");

    y = 0;
    do
    {
        if(!(y&15))service->busybox((int)(((long)y << 7L) / (long)height));
        ...
        */
        for(x=0; x<width/16; x++)
            output += set_16_pixels(line+x*16, output, dest_planes, planelen, 16);  /* 16 Pixel setzen... */

        if( width - (width/16)*16 >0 )
            output += set_16_pixels(line+x*16, output, dest_planes, planelen, width-(width/16)*16); /* 16 Pixel setzen... */
