...
/*	Åber eine MOD_ABILITY-Struktur.									*/
/*		05.06.1997 Olaf												*/
/*-----------------------------------------------------------------	*/
...
void transform_pic(void)
{
	...
	switch(button)
	{
		...
		default:			/*----------------- Name und Farbtiefe des Bildes einfÅgen --------*/
...
void do_transform(int conv_depth, int conv_dither, int conv_pal)
{
	...
	memset(&abs, 0x0, sizeof(MOD_ABILITY));				/* lîschen... */
...
int dither_destruktiv(int dest_depth, int dest_dither, int dest_pal)
{
	...
	if(dest_pal == CR_SYSPAL && dest_depth != Sys_info.bitplanes)
	{
		...
		{
			Dialog.winAlert.openAlert("Kein NCT-File fÅr die gewÑhlte Farbtiefe gefunden. Dithering nicht mîglich.", NULL, NULL, NULL, 1);
			return(-1);
		}
	}

	/*---------------- neue Strukturen zurechtbescheiûen -----*/
	...
	convpic->screen_pic = NULL;		/* problemlos zu ermîglichen */
	...
	/* Wird wohl in Zukunft wegfallen kînnen */
...
int autoreduce_image(void)
{
	...
	/* zu aller erst mal die Farben im Bild zÑhlen */
	rt = SMalloc(32769L);
	if(rt == NULL)
		return(M_MEMORY);
	...
	Dialog.busy.reset(0, "zÑhle Farben...");
	...
	for(t = 0; t < pixlen; t++)
	{
		...
		if(convpic->format_type==FORM_PIXELPAK)
		{
			...
			if(found == 0)		/* NICHT gefunden? -> Eintragen und FarbzÑhler erhîhen */
			{
				colcount++;
				rt[colcount] = R;
				gt[colcount] = G;
					...
					if(colcount <= 256)
						...
						if(colcount <= 32768L)
						{
							if(smurf_picture[active_pic]->depth == 24)
								if(Dialog.winAlert.openAlert("Das Bild kann nach 16 Bit Farbtiefe gewandelt werden. Folge kann aber ein QualitÑtsverlust sein!", "Abbruch", " OK ", NULL, 1) == 2)
