...
/*	                                                                            */
/*  Jedwede éhnlichkeit zu vorhandenen SchlÅmpfen, Schlumpfinen oder Personen   */
/*  oder sonstigen, fiktiven oder realen, Lebewesen ist von uns, nicht in       */
/*  bîser, sondern in guter Absicht, vorsÑtzlich eingebracht worden oder wurde  */
...
void main(int argc, const char *argv[])
{
	...
	set_startupdial("Initialisiere OberflÑche...");
	...
	/*
	 * Standardpfad fÅrs erste ôffnen der Fileselectorbox
	 */
...
/*------------------------ Generieren und FÅllen von Strukturen ---------------------------	*/
/*					WINDOW und SMURF_PIC fÅr neues oder geladenes Bild						*/
...
/* make_pic_window ------------------------------------------------
	Erzeugt eine WINDOW-Struktur fÅr das Bild mit der Nummer pic_to_make
	mit der Grîûe wid/hgt und dem Namen name. Ein Fenstertitel wird
	erzeugt, das Bild eingehÑngt und pflag auf 1 gesetzt.
	----------------------------------------------------------------*/
...
/*	Aktionen wie Radiobuttonbehandlung, UDOs, etc. ausgefÅhrt und die	*/
...
/*	Diese Funktion handled das Schlieûen eines Dialogs mit dem Closer		*/
/*	(was in Smurf einem Abbruch-Button gleichkommt) und setzt dessen		*/
/*	eventuelle Optionen zurÅck.												*/
/*-------------------------------------------------------------------------	*/
void check_windclose(int windnum)
{
	char str[4];
...
/* ******************************************************************/
/*						Dialog schlieûen							*/
/*	Fenster schlieûen und lîschen, aus der Liste nehmen Cursor aus, */
/* ******************************************************************/
...
/*
	fprintf(debug, "Smurf-Pfad: %s\n", Sys_info.standard_path);
	fprintf(debug, "AES Version %i\n", Sys_info.AES_version);
	fprintf(debug, "Screen: %iX%i\n", Sys_info.screen_width, Sys_info.screen_height);
	fprintf(debug, "  Farben: %i\n", Sys_info.Max_col);
	fprintf(debug, "  Bitplanes: %i\n", Sys_info.bitplanes);
	fprintf(debug, "Smurf VDI-Handle: %i\n", Sys_info.vdi_handle);
	fprintf(debug, "Smurf AES-ID: %i\n", Sys_info.app_id);
*/

	/*----------------- Globale Dienstfunktionen fÅr Module einhÑngen --------------*/
	...
	{
/*		fprintf(debug, "***** Keine Dithermodule!\n");*/
...
/*	fprintf(debug, "Dithermodule: %i\n", anzahl_dithermods);
	...
	{
/*		fprintf(debug, "***** Keine Editmodule!\n");*/
...
/*	fprintf(debug, "Editmodule: %i\n", Dialog.emodList.anzahl);*/
	...
	{
/*		fprintf(debug, "***** Keine Exportmodule!\n");*/
...
/*	fprintf(debug, "Systeminitialisierung abgeschlossen.\n");
	fclose(debug);*/
...
char *f_init_table(void)
{
	 ...
	 * Diese kann immer erzeugt werden, da ich die Farben immer weiû.
	 */
...
/* ----------------------------------------------------------------	*/
/* ------ BinÑrwerttabelle fÅr Palettenindex-Wert Umsetzung ------	*/
...
/*	diesem Standardblock wird dann der Wert binÑr ausgelesen. Das	*/
