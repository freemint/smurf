...
int load_config(void)
{
	...
	if(buf != NULL)
	{
		...
		{
			f_alert("Falsche Version der Konfigurationsdatei gefunden! Aus SicherheitsgrÅnden wird diese nicht eingeladen.", NULL, NULL, NULL, 1);
			SMfree(buf);
			return(-1);
		}
		...
		/* Zeiger retten, da er Åberschrieben wird */
		...
		strcpy(Sys_info.standard_path, stpath);	/* Åberschriebenen String wieder restaurieren */
...
void save_config(void)
{
	...
	if((fcback = Fcreate(path, 0)) >= 0)
	{
		...
		if(Fwrite(filehandle, 2, &ver) != 2)
			Dialog.winAlert.openAlert("Fehler beim Schreiben der Konfigurationsdatei! Mîglicherweise ist das Laufwerk voll.", NULL, NULL, NULL, 1);
